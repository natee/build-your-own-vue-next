# 实现渲染器
- [实现渲染器](#实现渲染器)
  - [Lesson 1 - 编译器和渲染器 API 初探](#lesson-1---编译器和渲染器-api-初探)
  - [Lesson 2 - 设计 VNode](#lesson-2---设计-vnode)
  - [Lesson 3 - 实现创建 VNode 的 h 函数](#lesson-3---实现创建-vnode-的-h-函数)
  - [Lesson 4 - 实现渲染器挂载的 mount 函数](#lesson-4---实现渲染器挂载的-mount-函数)
  - [Lesson5 - 实现patch函数](#lesson5---实现patch函数)

本章节将带你实现Vue 3的渲染器功能，分为如下四部分来讲解。

## Lesson 1 - 编译器和渲染器 API 初探
[点此学习](./LESSON-1-API.md)

## Lesson 2 - 设计 VNode
[点此学习](./LESSON-2-VNODE.md)

## Lesson 3 - 实现创建 VNode 的 h 函数
[点此学习](./LESSON-3-HFUNCTION.md)

## Lesson 4 - 实现渲染器挂载的 mount 函数
[点此学习](./LESSON-4-MOUNT.md)

## Lesson5 - 实现patch函数
[点此学习](./LESSON-5-PATCH.md)


[下一章 - Reactivity响应式设计](../chapter4/OVERVIEW.md)

