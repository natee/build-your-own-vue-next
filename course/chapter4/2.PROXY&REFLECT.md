...
Vue 3中我们将使用 ES6 中的 `Reflect` 和 `Proxy`。

**为什么（能）用 Proxy？**

![proxy](../../assets/chapter4-proxy.png)

![proxy-polyfill](../../assets/chapter4-proxy-polyfill.png)


在这样严峻的条件下，为什么 Vue 3还要使用 `Proxy` 呢？答案就是 **Vue 3不再支持IE浏览器**，所以可以用比较多的高级特性。

进入正题，我们先来看一下怎么输出一个对象的一个属性值，可以用下面这三种方法：
- 使用 `.` => `obj.a`
- 使用 `[]` => `obj['a']`
